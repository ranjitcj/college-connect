...
export default function AttendanceDashboard() {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      const response = await fetch(`/api/attendance?roll=${encodeURIComponent(rollNumber)}`);
  ...
  const processData = (data: { scores: any[]; }) => {
    ...
    const subjectEntries = data.scores.filter((item: { subject: string; }) => item.subject !== 'TOTAL');
    const totalEntry = data.scores.find((item: { subject: string; }) => item.subject === 'TOTAL');
    ...
    const subjects = subjectEntries.map((item: { subject: any; }) => item.subject);
    const attendanceValues = subjectEntries.map((item: { attendance: any; }) => item.attendance);
    
    // Generate colors for charts
    const colors = generateColors(subjectEntries.length);
    
    // Update summary
    const highest = subjectEntries.reduce((max: { attendance: number; }, item: { attendance: number; }) => item.attendance > max.attendance ? item : max, subjectEntries[0]);
    const lowest = subjectEntries.reduce((min: { attendance: number; }, item: { attendance: number; }) => item.attendance < min.attendance ? item : min, subjectEntries[0]);
  ...
  const getAlertVariant = (attendance: number) => {
    ...
    if (attendance >= 60) return 'bg-yellow-50 text-yellow-800 border-yellow-200';
    return 'bg-red-50 text-red-800 border-red-200';
  };

  const getStatusMessage = (attendance: number) => {
    if (attendance >= 75) return 'Your attendance is good! Keep it up.';
    if (attendance >= 60) return 'Your attendance needs improvement. Try to attend more classes.';
    ...
    <div className="container mx-auto py-8 px-4">
      <Card className="shadow-lg">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-bold">Student Attendance Dashboard</CardTitle>
          <CardDescription>Enter a roll number to view detailed attendance analysis</CardDescription>
        </CardHeader>
          ...
          {showResults && (
            <div className="max-w-4xl mx-auto">
              <div className="mb-6">
                ...
                <Progress value={attendanceSummary.totalAttendance} className="h-4" />
              </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <Card className="bg-white-50">
                            <CardHeader className="pb-2">
                              <CardTitle className="text-base font-medium">Overall</CardTitle>
                            </CardHeader>
                            <CardContent>
                              <p className="text-2xl font-bold">{attendanceSummary.totalAttendance.toFixed(1)}%</p>
                            </CardContent>
                          </Card>
                          
                          <Card className="bg-white-50">
                            <CardHeader className="pb-2">
                              <CardTitle className="text-base font-medium">Highest</CardTitle>
                            </CardHeader>
                            <CardContent>
                              <p className="text-2xl font-bold">{attendanceSummary.highest.subject}</p>
                              <p className="text-lg">{attendanceSummary.highest.attendance.toFixed(1)}%</p>
                            </CardContent>
                          </Card>
                          
                          <Card className="bg-white-50">
                            <CardHeader className="pb-2">
                              <CardTitle className="text-base font-medium">Lowest</CardTitle>
                            </CardHeader>
                            <CardContent>
                              <p className="text-2xl font-bold">{attendanceSummary.lowest.subject}</p>
                              <p className="text-lg">{attendanceSummary.lowest.attendance.toFixed(1)}%</p>
                            </CardContent>
                          </Card>
                        </div>
                        
                        <Alert className={`${getAlertVariant(attendanceSummary.totalAttendance)} border`}>
                          <AlertDescription>
                            {getStatusMessage(attendanceSummary.totalAttendance)}
                          </AlertDescription>
                        </Alert>
                      </div>
                    </CardContent>
